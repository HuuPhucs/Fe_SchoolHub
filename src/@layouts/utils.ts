...
const resolveNavLinkRouteFullPath = (link: NavLink, router: Router) => {
  if (!link.to)
    return null
  ...
  return router.resolve(link.to).fullPath
}

// const nestedMenuConfig = await MenuService.GetNestedMenuConfig().then(res => { return res })
...
export const isNavLinkActive = (link: NavLink, router: Router) => {
  ...
  const resolveRoutedFullPath = resolveNavLinkRouteFullPath(link, router)

  if (!resolveRoutedName)
    return false

  if (!resolveRoutedFullPath)
    return false

  // Comment this line to use original condition
  return currentPath === resolveRoutedFullPath || (resolveRoutedFullPath.startsWith('/tin-tuc') && currentPath.startsWith('/tin-tuc'))

  const result = matchedRoutes.some(route => {
